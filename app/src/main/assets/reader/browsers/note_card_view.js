...
NoteCardView.prototype.setNote = function (note) {
  ...
  if (note.text != undefined) {
    ...
    if (note.text.startsWith("</div>")) note.text = note.text.substr(6);
    if (note.text.endsWith(startDiv)) note.text = note.text.substr(0, note.text.length - startDiv.length);
  ...
  if (note.metadata.keywords.length > 0) {
    if (typeof note.metadata.keywords[Symbol.iterator] === 'function') {
      var _iterator = _createForOfIteratorHelper(note.metadata.keywords),
        _step;
      try {
  ...
  if (note.previews != undefined && !note.fromCache) {
    var _iterator2 = _createForOfIteratorHelper(note.previews),
      _step2;
    try {
      for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
        var preview = _step2.value;
        var img = document.createElement('img');
        if (!preview.startsWith("data:") && note.path != "untitleddonotedit.sqd")
          //fake notes don't need api
  ...
  if (note.media != undefined) {
    ...
    var _iterator3 = _createForOfIteratorHelper(note.media),
      _step3;
    try {
      var _loop2 = function _loop2() {
          var url = _step3.value;
          var audio = url.substr(url.lastIndexOf("/") + 1);
          if (!FileUtils.isFileAudio(audio)) return 1; // continue
