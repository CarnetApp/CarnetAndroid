...
(function ($) {
  $.widget("wgm.imgViewer", {
    ...
    _create: function _create() {
      ...
      self.zimg = $("<img />", {
        "src": self.img.src
      }).appendTo("#fullimg_container").wrap("<div class='viewport' />");
      var $zimg = $(self.zimg); //		the container or viewport for the image view

      self.view = $(self.zimg).parent();
      var $view = $(self.view); //		the pixel coordinate of the original image at the center of the viewport

      self.vCenter = {}; //		a flag used to decide if a mouse click is part of a drag or a proper click

      self.drag = false;
      self.pinch = false; //		a flag used to check the target image has loaded
      ...
      $img.one("load", function () {
        ...
        var width = $img.width(),
            height = $img.height(),
            offset = $img.offset(); //			cache the image padding information
    ...
    _bind_zoom_events: function _bind_zoom_events() {
      ...
      $zimg.on("pinch", function (ev) {
        ...
        if (!self.pinch) {
          ...
          self.pinchstart = {
            x: ev.gesture.center.x + scoff.x,
            y: ev.gesture.center.y + scoff.y
          };
    ...
    panTo: function panTo(relx, rely) {
      if (this.ready && relx >= 0 && relx <= 1 && rely >= 0 && rely <= 1) {
        ...
        this.update();
        return {
          x: this.vCenter.x / width,
          y: this.vCenter.y / height
        };
      } else {
        return null;
      }
    ...
    update: function update() {
      if (this.ready) {
        ...
        $(this.zimg).css({
          transform: "translate(" + xt + "px," + yt + "px) scale(" + 1 + "," + 1 + ")"
        });
        this.view.css({
