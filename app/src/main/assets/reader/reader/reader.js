...
Writer.prototype.displayMediaFullscreen = function (index) {
    ...
    this.fullscreenViewer.onclick = function () {
        if ($(toolbar).is(":visible")) $(toolbar).slideUp("fast");else $(toolbar).slideDown("fast");
        //  $(writer.fullscreenViewer).hide("slow")
    };
};
...
Writer.prototype.refreshMedia = function () {
    ...
    } else fs.readdir(tmppath + 'data/', function (err, dir) {
        if (err) {
            return;
        }
        writer.currentFullscreen = 0;
...
Writer.prototype.addMedia = function () {
    ...
    if (!isElectron) {
    ...
    } else FileOpener.selectFile(function (fileNames) {
        ...
        fs.readFile(filePath, 'base64', function read(err, data) {
            ...
            _require2('mkdirp').sync(tmppath + 'data/');
...
Writer.prototype.fillWriter = function (extractedHTML) {
    console.log("fill");
    if (isElectron) {
        var _require = _require2('electron'),
    ...
    this.oDoc.focus();
    if ((typeof app === "undefined" ? "undefined" : _typeof(app)) == 'object') app.hideProgress();
    resetScreenHeight();
...
Writer.prototype.exit = function () {
    if (window.self !== window.top) //in iframe
        parent.postMessage("exit", "*");else if ((typeof app === "undefined" ? "undefined" : _typeof(app)) === "object") app.postMessage("exit", "*");
};
