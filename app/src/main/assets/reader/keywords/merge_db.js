...
var KeywordDBMerger = function KeywordDBMerger(recentFolder, appUid) {
...
KeywordDBMerger.prototype.startMergin = function (onFinished) {
    ...
    fs.readdir(this.recentFolder, function (err, dir) {
        if (dir == undefined) {
            onFinished(false);
            return;
        }
...
KeywordDBMerger.prototype.mergeNext = function () {
    ...
    if (this.pos < this.dir.length) {
        if (this.dir[this.pos].startsWith(".")) {
            //skip
            merger.pos++;
            merger.mergeNext();
        }
        db.mergeDB(merger.recentFolder + "/" + this.dir[this.pos], function (hasChanged) {
            if (hasChanged) merger.hasChanged = hasChanged;
            merger.pos++;
            merger.mergeNext();
        });
