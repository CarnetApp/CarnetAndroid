...
public class http2 {
    ...
    public http2(Context ct) throws IOException{
	...
	private static final String[] SUBTITLES_ARRAY = { "idx", "smi", "ssa", "ass", "srr", "srt", "sub", "mpl", "txt"};
		...
		private void openInputStream() throws IOException{
			...
			try {
				...
				if(path!=null){
					...
					if(path.equals(getReaderPath())||path.startsWith("/assets/")){
						//return reader
						AssetManager assetManager = mContext.getAssets();
						Log.d("pathdebug","asking reader");
						...
						Log.d("pathdebug","asking something else "+path);

						ZipEntry entry = getZipEntry(path);
						is = getZipInputStream(entry);
	...
	public String getReaderUri(){
		...
		String url = "http://localhost:"+port+getReaderPath();

		return url;
	}

	public String getReaderPath(){
		String url = "/reader.html";
